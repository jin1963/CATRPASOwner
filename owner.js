// owner.js — CAT Owner Console (id ตรงกับ owner.html เวอร์ชันล่าสุด)
  ...
  try{
    ...
    try {
      ...
      if (ownerAddr.toLowerCase() !== account.toLowerCase()){
        el('ownerGate').textContent = 'ℹ️ บัญชีนี้ไม่ใช่ Owner — ยังส่งธุรกรรมได้ แต่สัญญาจะตรวจสิทธิ์เอง';
      } else {
        el('ownerGate').textContent = '✅ คุณคือ Owner — จัดการสัญญาได้';
      }
    } catch(e){
      el('ownerAddr').textContent = '-';
      el('ownerGate').textContent = 'ℹ️ อ่าน owner() ไม่ได้ — ดำเนินการต่อได้';
    }
...
async function loadPackages(){
  ...
  try{
    ...
    // สัญญานี้ใช้ index เริ่มจาก 0 (จากหน้าจอคุณมี #0,#1,...)
    for (let i=0;i<=count-1;i++){
      ...
      html += `
        <div class="pkg">
          ...
          <div class="row">
            <button class="btnFill" data-id="${i}" data-usdt="${uInRaw}" data-cat="${outRaw}" data-active="${p.active}">แก้ไข (เติมฟอร์ม)</button>
          </div>
        </div>
      `;
    }
...
async function setPackage(){
  try{
    ...
    if (!Number.isFinite(id)) return toast('กรุณาใส่ Package ID','err');
...
async function withdrawCAT(){
  try{
    ...
    // ใน ABI มี ownerWithdrawStakeToken(amount,to)
    if (!sale.methods.ownerWithdrawStakeToken)
      throw new Error('ไม่พบเมธอด ownerWithdrawStakeToken ในสัญญา');
...
// wire (รอ DOM พร้อมแล้วค่อยจับปุ่ม)
