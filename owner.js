// owner.js — CAT Owner Console (robust, owner()-optional)
...
function decimals() {
  return {
    ...
    CAT : window.DECIMALS?.CAT ?? window.DECIMALS?.KJC ?? 18,
  };
}
...
async function connect(){
  try{
    ...
    // อ่าน owner แบบปลอดภัย: ถ้าไม่มีฟังก์ชัน owner() จะข้าม
    try {
      const hasOwnerFn = !!sale.methods.owner;
      if (hasOwnerFn) {
    ...
    // แสดงสถานะให้รู้เฉย ๆ แต่จะไม่บล็อคปุ่ม ถ้าอ่าน owner ไม่ได้
    if (ownerAddr && ownerAddr.toLowerCase() !== account.toLowerCase()){
      el('ownerGate').textContent = 'ℹ️ ตรวจ owner บนเชนแล้ว: บัญชีนี้ไม่ใช่ Owner (ยังทำธุรกรรมได้ แต่สัญญาจะเป็นคนคัดสิทธิ์)';
    } else if (ownerAddr) {
      el('ownerGate').textContent = '✅ คุณคือ Owner — สามารถจัดการสัญญาได้';
    } else {
      el('ownerGate').textContent = 'ℹ️ สัญญาไม่เปิดให้เรียก owner() หรือ ABI ไม่พบฟังก์ชันนี้ — ใช้งานต่อได้';
    }
...
async function refreshStatus(){
  try{
    ...
    try {
      ...
      el('balKJC').textContent  = `${fmt(balC, decC)} CAT`;
    } catch(e) { el('balKJC').textContent='-'; console.warn('balanceOf CAT failed', e); }
...
async function loadPackages(){
  ...
  try{
    ...
    // ลองทั้งสองช่วง index (1..count) และ (0..count-1)
    ...
    if (rows.length===0 || rows.every(r=>!r.active)) {
      rows = [];
    ...
    for (const r of rows){
      html += `
        <div class="pkg">
          ...
          <div class="row">
            <button class="btnFill" data-id="${r.i}" data-usdt="${r.uInRaw}" data-kjc="${r.outRaw}" data-active="${r.active}">แก้ไข (เติมฟอร์ม)</button>
          </div>
        </div>
      `;
    }
    ...
    [...document.querySelectorAll('.btnFill')].forEach(b=>{
      b.addEventListener('click', ()=>{
        ...
        el('pkgKjc').value   = fmt(b.dataset.kjc,  decC); // input เดิม แต่เป็น CAT
...
async function setPackage(){
  try{
    // อย่าหยุดผู้ใช้ไว้ที่ frontend — ให้สัญญาตรวจ owner เอง
    const id   = Number((el('pkgId').value||'').trim());
    const uIn  = (el('pkgUsdt').value||'').trim();
    const tOut = (el('pkgKjc').value||'').trim();
...
async function withdrawKJC(){ // ปุ่มเดิม แต่ถอน CAT
  try{
    const to = (el('wTo').value||'').trim() || account;
    if (!web3.utils.isAddress(to)) return toast('ที่อยู่ปลายทางไม่ถูกต้อง','err');
